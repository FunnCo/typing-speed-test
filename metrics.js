...
/**
 * Сравнение слов по позициям, total = max(lenTyped, lenTarget)
 */
export function compareWordByPositions(typed, target) {
...
export function standardWpm(charsTyped, durationSec) {
  const minutes = durationSec / 60;
  if (!minutes) return 0;
  return Math.max(0, Math.floor((charsTyped / 5) / minutes));
}

export function wordWpm(wordsCorrect, durationSec) {
  const minutes = durationSec / 60;
  if (!minutes) return 0;
...
if (typeof window !== 'undefined') window.TypingMetrics = TypingMetrics;
